# Prospect Automation — n8n on Render + PhantomBuster + Claude
...
Claude scores candidates against the ICP rubric, results land in Google
Sheets.
...
- `n8n/02-ingest-webhook.json` — receives PhantomBuster's completion webhook, fetches the scraped candidates, scores each with Claude against the Sharp SSDI Tier 1/2/3 rubric, writes results to the Prospects sheet.
...
4. In n8n: import both `n8n/01-kickoff.json` and `n8n/02-ingest-webhook.json`, set Google Sheets/Anthropic/PhantomBuster credentials, replace the `YOUR_GOOGLE_SHEET_ID` placeholders, activate both workflows.
...
| Input: Client-specific prospecting rules | Phantom's `search` argument + the Tier 1/2/3 rubric in the Claude prompt (currently hardcoded to Sharp SSDI) |
