...
- **Per-company completion tracking**: `02-ingest-webhook.json`'s "Mark Company Done" node matches by `Company Name` and fires once per top-4 candidate written for that company, setting `Status = Done`. Filter the Companies sheet on `Status` to see which companies actually got prospects written.
- **Orphaned companies (the "Success / Input already processed" problem) are fixed.** PhantomBuster's Watcher Mode dedups at the *scrape* level — it won't rescrape a company already seen — but that's independent of whether our pipeline successfully processed it downstream. If a run's webhook/scoring chain failed for a company after PhantomBuster had already scraped it, that company would never be offered again through the per-run webhook, staying stuck forever. Fixed by always reprocessing against the full accumulated dataset: on every webhook fire, "Fetch Agent Info" → "Fetch Full Accumulated Result" pulls *every* lead this agent has ever scraped (via its S3 result file, not just this run's delta), "Read All Companies" reads the Companies sheet, and "Filter To Pending Companies" cross-references the two — processing every company still not `Done`, regardless of which run originally scraped it.
  - The output filename is read from the agent's own configuration (`argument.csvName` in "Fetch Agent Info"'s response) rather than assumed — a real run confirmed this agent's output file had been renamed to `result5` (Behavior → "Name your results file" in PhantomBuster), and a hardcoded `result.json` was silently pulling a stale, pre-rename file.
  - The file *format* is also handled automatically now, not just the name: "Fetch Result (Try JSON)" tries `{csvName}.json` first; if that errors (`onError: continueErrorOutput`, e.g. a 403/404 because this agent's Results File is actually CSV), it falls through to "Fetch Result (CSV Fallback)", which fetches `{csvName}.csv` as raw text and parses it in "Parse CSV Result". Both paths converge on "Candidates Ready", which always exposes the same `{candidates: [...]}` shape regardless of which format the agent actually used — "Filter To Pending Companies" reads from `$('Candidates Ready')` rather than hardcoding one format. Whichever Results File setting this (or any future) agent uses, no manual URL/extension guessing is needed.
  - Enable **Watcher Mode** on the PhantomBuster agent (Behavior step) so a phantom run interrupted partway through a multi-company scrape resumes with unscraped companies next launch instead of restarting from zero — this is scrape-level resumability, complementary to the pipeline-level fix above.
  - **Field-name normalization**: PhantomBuster's own JSON export uses camelCase keys (`companyName`, `fullName`, `profileUrl`, `titleDescription`, `location`...), but the CSV/console export uses Title Case column names with spaces (`Company Name`, `Full Name`, `Profile Url`, `Title Description`, `Location`...) — confirmed against a real console export. "Filter To Pending Companies" now normalizes every candidate through both naming styles before filtering/scoring, so it works regardless of which export shape the agent actually produced. It also drops error-shaped entries PhantomBuster writes for bad input rows (e.g. `{"error": "Not a Company URL", "query": "..."}` — this happens when a Companies sheet row has a personal profile or Sales Navigator lead URL instead of a company page URL) instead of letting them flow downstream as fake candidates.
